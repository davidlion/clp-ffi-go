...
//   - success: valid [*Reader] and nil == error
//   - error: [*Reader] is nil and error propagated from
//     [DeserializePreamble] or [Reader.read]
...
//   - error propagated from [DeserializeLogEvent] or [Reader.read]
...
// fill_buf shifts the remaining valid IR in [Reader.buf] to the front and then
// calls [Reader.read] to fill the remainder with more IR. Before reading into
// the buffer, it is grown by 1.5x if more than 1/4th of it is unconsumed IR.
// Forwards the return of [Reader.read].
func (self *Reader) fill_buf() (int, error) {
...
// read. On success nil is returned. On failure an error is forwarded fro
